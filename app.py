...
# ============================================================
# DATABASE INITIALIZATION + MIGRATION (KPI4 -> KPI6)
# ============================================================
def initialize_database():
    """Initialize + migrate database WITHOUT dropping existing data"""
    try:
        st.info("🔄 Checking / Migrating database (no data loss)...")
        ...
        # Fresh installs will get KPI1..KPI6
        execute_query("""
            CREATE TABLE IF NOT EXISTS kpi_entries (
                ...
                kpi5 INTEGER NOT NULL DEFAULT 50,
                kpi6 INTEGER NOT NULL DEFAULT 50,
        ...
        # Migration for existing DBs: add KPI5/KPI6 columns safely
        execute_query("ALTER TABLE kpi_entries ADD COLUMN IF NOT EXISTS kpi5 INTEGER NOT NULL DEFAULT 50")
        execute_query("ALTER TABLE kpi_entries ADD COLUMN IF NOT EXISTS kpi6 INTEGER NOT NULL DEFAULT 50")

        # Default settings (no overwrite)
        ...
        for kpi_key, label in default_labels:
            execute_query("""
                INSERT INTO kpi_master(kpi_key, kpi_label) VALUES (%s, %s)
                ON CONFLICT (kpi_key) DO NOTHING
            """, [kpi_key, label])

        # KPI weights: keep old behavior (kpi1..4=25) and add kpi5/kpi6=0
        for kpi_key, w in [("kpi1", 25), ("kpi2", 25), ("kpi3", 25), ("kpi4", 25), ("kpi5", 0), ("kpi6", 0)]:
            execute_query("""
                INSERT INTO kpi_weights(kpi_key, weight) VALUES (%s, %s)
                ON CONFLICT (kpi_key) DO NOTHING
            """, [kpi_key, w])

        # Rating rules default
        execute_query("""
        ...
        st.success("✅ Database ready (data preserved) + KPI5/KPI6 enabled!")
        return True
    except Exception as e:
        st.error(f"❌ Database initialization/migration error: {str(e)}")
        return False

# Initialize database (preserve data + migrate)
...
def get_kpi_labels():
    ...
    return (
        labels.get("kpi1", "KPI 1"),
        labels.get("kpi2", "KPI 2"),
        labels.get("kpi3", "KPI 3"),
        labels.get("kpi4", "KPI 4"),
        labels.get("kpi5", "KPI 5"),
        labels.get("kpi6", "KPI 6"),
    )
...
def calc_weighted_score(k1, k2, k3, k4, k5, k6):
    w1, w2, w3, w4, w5, w6 = get_kpi_weights()
    return round((k1*w1 + k2*w2 + k3*w3 + k4*w4 + k5*w5 + k6*w6) / 100.0, 2)
...
def get_active_departments():
    rows = execute_query(
        "SELECT department_name FROM departments WHERE is_active=TRUE ORDER BY department_name",
        fetch=True
    ) or []
...
def get_all_departments():
    return execute_query(
        "SELECT id, department_name, is_active, created_at FROM departments ORDER BY department_name",
        fetch=True
    ) or []
...
# ============================================================
# QUERY KPI DATA (now KPI1..KPI6)
# ============================================================
q = """
SELECT id, employee_name, department,
       kpi1, kpi2, kpi3, kpi4, kpi5, kpi6,
       total_score, rating, created_at, COALESCE(created_by, 'system') as created_by
...
if 'dept_filter' in globals() and dept_filter != "All" and user_role == "admin":
    q += " AND department=%s"
    p.append(dept_filter)
if 'emp_filter' in globals() and emp_filter != "All" and user_role != "employee":
    q += " AND employee_name=%s"
    p.append(emp_filter)
if 'rating_filter' in globals() and rating_filter != "All":
    q += " AND rating=%s"
    p.append(rating_filter)
if 'date_range' in globals() and isinstance(date_range, (list, tuple)) and len(date_range) == 2:
...
kpi1_lbl, kpi2_lbl, kpi3_lbl, kpi4_lbl, kpi5_lbl, kpi6_lbl = get_kpi_labels()
...
if menu == "Dashboard":
    ...
    if user_role == "admin":
        ...
        active_emp = len(execute_query(
            "SELECT id FROM employees WHERE is_active=TRUE AND department=%s",
            [user_department], fetch=True
        ) or [])
        active_dept = 1
    else:
        active_emp = 1
    ...
    if len(df) > 0:
        ...
        if user_role != "employee":
            ...
            with col_p2:
                ...
                fig = px.bar(
                    top_emp, x="Score", y="Employee", orientation='h',
...
if menu == "Entry":
    ...
    with st.form("kpi_form", clear_on_submit=True):
        ...
        with c5:
            v5 = st.number_input(f"🧩 {kpi5_lbl}", 1, 100, 50, 1)
        with c6:
            v6 = st.number_input(f"🛡️ {kpi6_lbl}", 1, 100, 50, 1)

        preview_score = calc_weighted_score(v1, v2, v3, v4, v5, v6)
    ...
    if submit:
        if emp and dept:
            score = calc_weighted_score(v1, v2, v3, v4, v5, v6)
            rating = calc_rating(score)
            now = datetime.now()
            month = now.strftime("%Y-%m")

            result = execute_query("""
                INSERT INTO kpi_entries (employee_name, department,
                                        kpi1, kpi2, kpi3, kpi4, kpi5, kpi6,
                                        total_score, rating, created_at, entry_month, created_by)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """, [emp, dept, v1, v2, v3, v4, v5, v6, score, rating, now, month, username])
...
if menu in ["Records", "My Records"]:
    ...
    if user_role in ["admin", "manager"] and len(df) > 0 and get_setting("allow_edit_delete", "1") == "1":
        ...
        with col_e1:
            ...
            with c5:
                ek5 = st.number_input(kpi5_lbl, 1, 100, int(row["KPI5"]), key="ek5")
            with c6:
                ek6 = st.number_input(kpi6_lbl, 1, 100, int(row["KPI6"]), key="ek6")
        ...
        with col_e2:
            ...
            new_score = calc_weighted_score(ek1, ek2, ek3, ek4, ek5, ek6)
        ...
        with col_b1:
            if st.button("💾 Update", use_container_width=True, type="primary"):
                execute_query("""
                    UPDATE kpi_entries
                    SET kpi1=%s, kpi2=%s, kpi3=%s, kpi4=%s, kpi5=%s, kpi6=%s,
                        total_score=%s, rating=%s,
                        updated_by=%s, updated_at=%s
                    WHERE id=%s
                """, [ek1, ek2, ek3, ek4, ek5, ek6, new_score, new_rating, username, datetime.now(), rec_id])
...
if menu == "Reports":
    ...
    if len(df) > 0:
        ...
        if chart_type == "Bar":
            ...
            if y in rep.columns and pd.api.types.is_numeric_dtype(rep[y]):
                fig.update_traces(texttemplate='%{text:.2f}', textposition='outside')
        ...
        st.download_button("📥 Download", csv, f"report_{sel_month}.csv", "text/csv")
    else:
                ...
                with col_b2:
                    if st.button("🗑️ Delete", use_container_width=True) and user_role == "admin":
                        entries = execute_query(
                            "SELECT COUNT(*) FROM kpi_entries WHERE employee_name=%s",
...
if menu == "Departments":
    ...
    with tab2:
        ...
        if depts:
            ...
            for idx, row_ in dept_df.iterrows():
                emp_count = execute_query(
                    "SELECT COUNT(*) FROM employees WHERE department=%s",
                    [row_["Name"]], fetch_one=True
                )
...
# ============================================================
# USERS (Admin only) - role editable (enhanced)
# ============================================================
if menu == "Users":
    if not require_auth("admin"):
        st.stop()
    ...
    with tab2:
        ...
        if users:
            user_df = pd.DataFrame(users, columns=["Username", "Name", "Role", "Employee", "Dept", "Active", "Last Login"])
            user_df["Status"] = user_df["Active"].apply(lambda x: "✅" if x else "❌")

            st.dataframe(
                user_df[["Username", "Name", "Role", "Employee", "Dept", "Status", "Last Login"]],
            ...
            if user_sel:
                ...
                with col1:
                    ...
                    # ✅ Role-based employee link editable
                    active_emps = get_active_employees()
                    emp_list = [e[0] for e in active_emps]
                    if edit_role in ["employee", "manager"]:
                        edit_emp = st.selectbox(
                            "Link Employee", [""] + emp_list,
                            index=([""] + emp_list).index(uemp) if uemp in emp_list else 0,
                            key="u_emp_link"
                        )
                        if edit_emp:
                            emp_dept = [e[1] for e in active_emps if e[0] == edit_emp]
                            edit_dept = emp_dept[0] if emp_dept else None
                            st.info(f"Dept: **{edit_dept}**")
                        else:
                            edit_dept = None
                    else:
                        edit_emp = None
                        edit_dept = None
                        st.info("Admin role: employee/dept link not required.")
                ...
                with col_b1:
                    if st.button("💾 Update", use_container_width=True, type="primary"):
                        execute_query("""
                            UPDATE users
                            SET full_name=%s, role=%s, is_active=%s, employee_name=%s, department=%s
                            WHERE username=%s
                        """, [edit_fullname, edit_role, edit_active, edit_emp, edit_dept, uname])
...
if menu == "Audit Log":
    ...
    if logs:
        ...
        st.download_button("📥 Export", csv, f"audit_{datetime.now().strftime('%Y%m%d')}.csv", "text/csv")
    else:
        st.info("📌 No logs")
...
# ============================================================
# SETTINGS (Admin only) - now KPI1..KPI6
# ============================================================
if menu == "Settings":
    if not require_auth("admin"):
        st.stop()
    ...
    with tab1:
        ...
        k1, k2, k3, k4, k5, k6 = get_kpi_labels()
        ...
        n5 = st.text_input("KPI 5", value=k5, key="kl5")
        n6 = st.text_input("KPI 6", value=k6, key="kl6")
        ...
        if st.button("💾 Save Labels", use_container_width=True, type="primary"):
            ...
            execute_query("UPDATE kpi_master SET kpi_label=%s WHERE kpi_key='kpi5'", [n5.strip() or "KPI 5"])
            execute_query("UPDATE kpi_master SET kpi_label=%s WHERE kpi_key='kpi6'", [n6.strip() or "KPI 6"])
    ...
    with tab2:
        ...
        # refresh labels in case admin just updated
        kpi1_lbl, kpi2_lbl, kpi3_lbl, kpi4_lbl, kpi5_lbl, kpi6_lbl = get_kpi_labels()
        ...
        nw5 = st.number_input(f"{kpi5_lbl}", 0, 100, w5, key="w5")
        nw6 = st.number_input(f"{kpi6_lbl}", 0, 100, w6, key="w6")

        total = nw1 + nw2 + nw3 + nw4 + nw5 + nw6
        if total == 100:
        ...
        if st.button("💾 Save Weights", use_container_width=True, type="primary"):
            if total == 100:
                ...
                execute_query("UPDATE kpi_weights SET weight=%s WHERE kpi_key='kpi5'", [nw5])
                execute_query("UPDATE kpi_weights SET weight=%s WHERE kpi_key='kpi6'", [nw6])
                log_action(username, "UPDATE_WEIGHTS", f"{nw1},{nw2},{nw3},{nw4},{nw5},{nw6}")
                st.success("✅ Saved!")
                st.rerun()
            else:
